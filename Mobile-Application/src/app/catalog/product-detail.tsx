...
export default function ProductDetailScreen() {
  ...
  const fetchDetail = async () => {
    try {
      ...
      Alert.alert('Error', 'Gagal memuat detail bunga.');
    } finally {
      setLoading(false);
    }
  };
        ...
        <ThemedView style={styles.content}>
          <ThemedView style={styles.headerInfo}>
            ...
            <ThemedView style={styles.badgeRow}>
               <ThemedView style={[styles.badge, { backgroundColor: theme.tint + '15' }]}>
                <ThemedText style={{ color: theme.tint, fontSize: 12 }}>{data.category?.name || 'Bunga'}</ThemedText>
              </ThemedView>
              <ThemedText themeColor="textSecondary" type="small">Stok: {data.stock || 0}</ThemedText>
            </ThemedView>
          </ThemedView>
          ...
          <ThemedView style={styles.section}>
            <ThemedText type="smallBold" style={styles.sectionTitle}>Deskripsi Produk</ThemedText>
            <ThemedText style={styles.description}>
              {data.description || 'Tidak ada deskripsi tersedia untuk produk ini.'}
            </ThemedText>
          </ThemedView>
          ...
          <ThemedView style={styles.section}>
            <ThemedText type="smallBold" style={styles.sectionTitle}>Toko / Organizer</ThemedText>
            <ThemedView style={styles.organizerCard}>
              <Ionicons name="storefront-outline" size={24} color={theme.icon} />
              <View style={styles.organizerInfo}>
                <ThemedText type="defaultSemiBold">{data.organizer?.name || 'Florist'}</ThemedText>
