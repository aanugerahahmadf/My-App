...
export default function ProductDetailScreen() {
  ...
  const fetchDetail = useCallback(async () => {
    ...
    setError(null);
    try {
      const res: any = await apiGet(API.PRODUCTS.DETAIL(parseInt(id, 10)));
      setItem(res.data || res || null);
      const checkRes: any = await apiGet(API.WISHLIST.CHECK(parseInt(id, 10))).catch(() => null);
      if (checkRes) setIsWishlist(checkRes.is_wishlisted || false);
    } catch (e: any) {
      setError(e.message || t('Failed to load products'));
    } finally {
      setLoading(false);
    }
  }, [id, t]);
  ...
  const toggleWishlist = async () => {
    try {
      await apiPost(API.WISHLIST.TOGGLE, { product_id: parseInt(id!, 10) });
  ...
  const addToCart = async () => {
    try {
      ...
      Alert.alert(t('Added to Cart'), t('Product has been added to your cart.'));
    } catch (e: any) {
  ...
  const chatAdmin = async () => {
    try {
      ...
      await apiPost(API.CHAT.SEND, {
        inbox_id: inboxId,
        message: t("Hi admin, I'm interested in this product: {name}").replace('{name}', item?.name || ''),
      });
    ...
    return (
      <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]}>
        <View style={[styles.header, { borderBottomColor: colors.backgroundSelected }]}>
          ...
          <Text style={[styles.headerTitle, { color: colors.text }]}>{t('Product')}</Text>
          <View style={{ width: 40 }} />
        </View>
        <View style={styles.center}>
          <Ionicons name="alert-circle-outline" size={48} color="#ef4444" />
          <Text style={[styles.errorText, { color: colors.text }]}>{error || t('Product not found')}</Text>
        </View>
      </SafeAreaView>
    );
  }
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        {item.category && (
          <View style={[styles.badge, { backgroundColor: colors.backgroundElement }]}>
            <Text style={[styles.badgeText, { color: colors.textSecondary }]}>{item.category.name}</Text>
          </View>
        )}
        ...
        <View style={styles.priceRow}>
          {item.discount_price ? (
            <>
              <Text style={styles.finalPrice}>Rp {finalPrice.toLocaleString('id-ID')}</Text>
        ...
        {item.stock > 0 && (
          <Text style={[styles.stockText, { color: colors.textSecondary }]}>
            {t('Stock: {n}').replace('{n}', String(item.stock))}
          </Text>
        )}
...
const styles = StyleSheet.create({
  ...
  badge: { alignSelf: 'flex-start', borderRadius: 8, paddingHorizontal: 10, paddingVertical: 4, marginBottom: Spacing.two },
  ...
  stockText: { fontSize: 13, marginBottom: Spacing.three },
  sectionTitle: { fontSize: 16, fontWeight: '700', marginBottom: Spacing.two, marginTop: Spacing.one },
